package com.ignoreos.jvmqq.Package;
import java.util.*;
import java.awt.image.*;

import com.ignoreos.jvmqq.Message.PictureStore;
import com.ignoreos.jvmqq.QQGlobal;
import com.ignoreos.jvmqq.TLV.TLVFactory;
import com.ignoreos.jvmqq.Utils.Crypter;
import com.ignoreos.jvmqq.sdk.MessageFactory;
import com.ignoreos.jvmqq.QQUser;
import com.ignoreos.jvmqq.Utils.ByteBuilder;
import com.ignoreos.jvmqq.Utils.Util;
import com.ignoreos.jvmqq.Utils.ZLibUtils;
...
public class SendPackageFactory
{
	...
	public static byte[] get00ba(QQUser user, String code)
	{
		...
		else
		{
			...
			body_builder.writebytesbylength(user.TXProtocol.BufSigPic);
			//输入验证码后清空图片流
	...
	public static byte[] get0002(QQUser user, MessageFactory message){
		...
		switch(message.message_type){
			case 0:
				{
					byte[] message_to_send = Util.constructmessage(user,message.Message.getBytes());
			...
			case 1:
				{
					byte[] message_to_send = ZLibUtils.compress(message.Message.getBytes());
	...
	public static byte[] sendpic(QQUser user, MessageFactory message){
		Util.log("[发送包] 命令: 00 02");
		ByteBuilder builder = new ByteBuilder();
		builder.writebytes(QQGlobal.QQHeaderBasicFamily);
		builder.writebytes(user.TXProtocol.CMainVer);
			...
			case 0:
				{
					byte[] message_to_send = Util.constructmessage(user,message.Message.getBytes());
					body_builder.writelong(user.QQ);
					body_builder.writelong(message.Friend_uin);
                    body_builder.writebytes(new byte[]
											{
	...
	public static byte[] get0388(QQUser user, MessageFactory message){
		...
		ByteBuilder body_builder=new ByteBuilder();
		BufferedImage img = Util.get_img(message.Message);
		long width = img.getWidth(); // 图片的宽度
		long height = img.getHeight(); // 图片的高度
		byte[] img_byte = Util.Bufferedimg_tobytes(img,message.Message.split("[.]")[message.Message.split("[.]").length-1]);
		long img_length = Util.getfilelength(message.Message);
		byte[] md5 = Util.str_to_byte(Util.GetMD5HashFromFile(message.Message));
		body_builder.writebytes(new byte[] {0x00,0x00,0x00,0x07});

		ByteBuilder img_builder=new ByteBuilder();
		img_builder.writebytes(Util.str_to_byte ("5A"));
		img_builder.writebyte ((byte)0x08);
		img_builder.writebytes (Util.str_to_byte(Util.PB_toLength (message.Group_uin)));
		...
		store.pictureid = seq;
		store.data = img_byte;
		store.Group =message.Group_uin;
		store.File = message.Message;
		user.imgs.add(store);
		return builder.getdata();
	}
